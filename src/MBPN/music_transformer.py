...
import random
from st_gcn_aaai18 import st_gcn_baseline
    ...
    def __init__(
        ...
        if rnn is not None:
            ...
            # RPR Transformer
                ...
                self.encoder_layer = TransformerEncoderLayerRPR(self.d_model, self.nhead, self.d_ff, self.dropout,
                                                           er_len=self.encoder_max_seq)
                self.encoder = TransformerEncoderRPR(self.encoder_layer, self.num_encoder_layers, self.encoder_norm)
                # self.decoder_norm = nn.LayerNorm(self.d_model)
                # self.decoder_layer = TransformerDecoderLayerRPR(
                #     self.d_model, self.nhead, self.d_ff, self.dropout, er_len=self.decoder_max_seq
                # )
                # self.decoder = TransformerDecoderRPR(
                #     self.decoder_layer, self.num_decoder_layers, norm=self.decoder_norm
                # )
                # self.transformer = nn.Transformer(
                #     d_model=self.d_model, nhead=self.nhead, num_encoder_layers=self.num_encoder_layers,
                #     num_decoder_layers=self.num_decoder_layers, dropout=self.dropout,  # activation=self.ff_activ,
                #     dim_feedforward=self.d_ff, custom_decoder=self.decoder, custom_encoder=self.encoder
                # )
...
def music_transformer_dev_baseline(
    ...
    pose_net = st_gcn_baseline(
        in_channels, d_model, layers=layers, layout=layout, dropout=dropout, encoder_max_seq = encoder_max_seq
    )
