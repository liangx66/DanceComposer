...
class ST_GCN_18(nn.Module):
    ...
    def __init__(self,
        ...
        fc_in_ch = layer_cfg[:num_layer-1][-1][0][1]    # 256
        ...
        self.fcn = nn.Sequential(
                        nn.Conv2d(fc_in_ch, num_class, kernel_size=1),    # nn.Conv2d(in_channel=256, out_channel=512, kernel_size=1)
                        nn.BatchNorm2d(num_class),
                    )
        self.encoder_max_seq = encoder_max_seq
        gcn_t = encoder_max_seq//4
        self.cn = nn.Sequential(
                        nn.Conv2d(gcn_t, encoder_max_seq, kernel_size=1),
                        nn.BatchNorm2d(encoder_max_seq),
                    )
...
def st_gcn_baseline(in_channels: int,  out_channels: int, layers: int, layout='body25', dropout=0.1, encoder_max_seq = 360):
    graph_cfg = {
        "layout": layout,
        "strategy": "spatial"
    }
